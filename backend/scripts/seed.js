...
async function seed() {
  ...
  try {
    ...
    await client.query(`
      INSERT INTO users (id,name,email,password_hash,role,phone,avatar,status,business,business_type,gst,location) VALUES
    ...
    await client.query(`
      INSERT INTO spaces (id,name,type,location,floors,area,status,owner_approved,admin_id,created_at) VALUES
      ...
      ON CONFLICT (id) DO NOTHING
    `);
    await client.query(`SELECT setval('spaces_id_seq', (SELECT MAX(id) FROM spaces))`);

    console.log('🌱  Seeding units…');
    await client.query(`
      INSERT INTO units (id,space_id,floor,number,type,width,height,price_daily,price_weekly,price_monthly,status,pos_x,pos_y,w,h) VALUES
    ...
    await client.query(`
      INSERT INTO bookings (id,unit_id,space_id,tenant_id,from_date,to_date,days,total,status,payment_status) VALUES
