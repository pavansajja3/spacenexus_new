...
function SpaceFormModal({ onSave, onClose, saving }) {
  ...
  const submit = () => {
    ...
    onSave({
      ...
      area: Number(f.area) || null,
    });
  };

  return (
    <Modal
      title="➕ Register New Space"
      subtitle="Space will be submitted to Super Owner for approval before going live"
      onClose={onClose}
      width={560}
    >
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        ...
        <Select
          ...
          value={f.type}
          onChange={s('type')}
          options={['Mall', 'Theatre', 'Bus Stand', 'Transit Hub', 'Exhibition Center', 'Other']}
        />
...
export default function Spaces({ spaces, setSpaces, units, role, onViewBlueprint, notify }) {
  ...
  const handleSave = async (data) => {
    try {
      ...
      const newSpace = response.space || response;
      ...
      notify(
        role === 'super_owner'
          ...
          : 'Space submitted for Super Owner approval! 📋'
      );
    } catch (err) {
      console.error('Create space failed:', err);
      notify(err.message || 'Failed to create space ❌', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleApprove = async (id) => {
    try {
      const response = await api.spaces.approve(id);
      const updated = response.space || response;

      setSpaces((prev) => prev.map((s) => (s.id === id ? updated : s)));
      notify('Space approved — now live for booking! ✅');
    } catch (err) {
      console.error('Approve space failed:', err);
      notify(err.message || 'Failed to approve space ❌', 'error');
    }
  };

  const handleReject = async (id) => {
    try {
      const response = await api.spaces.reject(id);
      const updated = response.space || response;

      setSpaces((prev) => prev.map((s) => (s.id === id ? updated : s)));
      notify('Space request rejected.', 'warn');
    } catch (err) {
      console.error('Reject space failed:', err);
      notify(err.message || 'Failed to reject space ❌', 'error');
    }
  };
  ...
  return (
    <div>
      <PageHeader
        title="Spaces"
        sub={`${spaces.length} commercial spaces registered`}
        action={
          <>
            <select
              value={typeF}
              ...
              style={{
                ...
                borderRadius: 10,
                padding: '8px 14px',
                color: typeF ? C.text : C.dim,
                fontSize: 13,
                outline: 'none',
              }}
            >
              <option value="">All Types</option>
              {['Mall', 'Theatre', 'Bus Stand', 'Transit Hub', 'Exhibition Center', 'Other'].map((t) => (
      ...
      {role === 'super_owner' && spaces.filter((s) => s.status === 'pending').length > 0 && (
        ...
        >
          ⚠️ {spaces.filter((s) => s.status === 'pending').length} space(s) awaiting your approval —
          approve to make them available for tenant bookings.
        </div>
      )}

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
        {filtered.map((s) => {
          const uAll = units.filter(
            (u) => u.spaceId === s.id || u.space_id === s.id
          );
            ...
            <Card key={s.id} onClick={() => onViewBlueprint(s)} sx={{ padding: 22 }}>
              ...
              >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
                  ...
                  {s.owner_approved || s.ownerApproved ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 14 }}>
                {[
                  ['Type', s.type],
                  ['Floors', s.floors],
                  ['Area', `${Number(s.area || 0).toLocaleString()} m²`],
                  ['Units', uAll.length],
                ].map(([l, v]) => (
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <div style={{ height: 4, background: C.border, borderRadius: 2 }}>
                    <div
                      style={{
                        height: '100%',
                        width: `${uAll.length ? (uBooked / uAll.length) * 100 : 0}%`,
                        background: C.red,
                        borderRadius: 2,
              ...
              <div style={{ display: 'flex', gap: 8 }} onClick={(e) => e.stopPropagation()}>
                <Btn size="sm" variant="ghost" onClick={() => onViewBlueprint(s)} style={{ flex: 1 }}>
                  🗺️ Blueprint
                </Btn>

                {role === 'super_owner' && s.status === 'pending' && (
                  <>
                    <Btn size="sm" variant="success" onClick={() => handleApprove(s.id)} style={{ flex: 1 }}>
                      ✓ Approve
                    </Btn>
                    <Btn size="sm" variant="danger" onClick={() => handleReject(s.id)}>
                      ✕
                    </Btn>
                  </>
                )}
          ...
          <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: 70, color: C.dim }}>
            ...
            {canAdd && (
              <div style={{ fontSize: 13, marginTop: 6 }}>
                Click "+ Add Space" to register a new commercial space
              </div>
            )}
          </div>
        )}
