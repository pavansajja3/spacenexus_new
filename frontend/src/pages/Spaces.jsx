  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 22 }}>
        <Btn onClick={handleSubmit} full disabled={saving}>
...
export default function Spaces({ spaces, setSpaces, units, role, onViewBlueprint, notify }) {
  ...
  const handleSave = async (data) => {
    try {
      const res = await api.spaces.create(data);
      ...
      notify(err.message || 'Failed to create space', 'warn');
    }
  };

  const handleApprove = async (id) => {
    try {
      const res = await api.spaces.approve(id);

      setSpaces((prev) =>
        prev.map((s) => (s.id === id ? res.space : s))
      );

      notify('Space approved — now live for booking! ✅');
    } catch (err) {
      notify(err.message || 'Failed to approve space', 'warn');
    }
  };

  const handleReject = async (id) => {
    try {
      const res = await api.spaces.reject(id);
      ...
      notify(err.message || 'Failed to reject space', 'warn');
    }
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
        {filtered.map((s) => {
          const uAll = units.filter((u) => (u.spaceId ?? u.space_id) === s.id);
            ...
            <Card key={s.id} onClick={() => onViewBlueprint(s)} sx={{ padding: 22 }}>
              ...
              >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
                  ...
                  {(s.ownerApproved ?? s.owner_approved) ? (
